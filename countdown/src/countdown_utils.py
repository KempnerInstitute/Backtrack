...
def parse_trajectory_qwen(search_path):
  # Remove speical tokens "START" and "END"
  processed_path = re.sub(r'\b(START|END)\b', '', search_path)
   
  # Fix number formatting: Remove spaces between digits within numbers
  processed_path = re.sub(r'(?<=\d) (?=\d)', '', processed_path)

  # Add new line before specified keywords
  keywords = ["Current", "Exploring", "Moving", "Generated"]
  for keyword in keywords:
      processed_path = re.sub(rf"(?<!\n)({keyword})", r"\n\1", processed_path)

  # Remove space before punctuation (":", ",", "#")
  processed_path = re.sub(r'\s+([:,#])', r'\1', processed_path)
  
  # Add new line before "a, b unequal: No Solution" 
  processed_path = re.sub(r'(\d{2,}), (\d{2,}) unequal: No Solution', r'\n\1,\2 unequal: No Solution', processed_path)

  # Add new line before "a, b equal: Goal Reached" 
  processed_path = re.sub(r'(\d{2,}), (\d{2,}) equal: Goal Reached', r'\n\1,\2 equal: Goal Reached', processed_path)

  # Add new line before "a, b equal" 
  processed_path = re.sub(r'(\d{2,}), (\d{2,}) equal', r'\n\1,\2 equal', processed_path)

  # Add new line before "a, b unequal" 
  processed_path = re.sub(r'(\d{2,}), (\d{2,}) unequal', r'\n\1,\2 unequal', processed_path)

  # Add new line before "[a, b] Goanl unequal: No Solution"
  processed_path = re.sub(r'(\[ \d{1,3}, \d{1,3}\ ] Goal unequal: No Solution)', r'\n\1', processed_path)
  
  # Remove space before and after single quotation marks "'"
  processed_path = re.sub(r"\s*'\s*", "'", processed_path)

  # Remove space after "[" and before "]"
  processed_path = re.sub(r'\[\s+', '[', processed_path)  # Remove space after "["
  processed_path = re.sub(r'\s+\]', ']', processed_path)  # Remove space before "]"

  # Remove space before and after mathematical operators ("+", "-", "*", "/", "=")
  processed_path = re.sub(r'\s*([+\-*/=])\s*', r'\1', processed_path)

  # Replace occurrences of ":  [" with ": ["
  processed_path = re.sub(r':\s+\[', ': [', processed_path)

  # Fix "Current State: XXX: [" → "Current State: XXX:["
  processed_path = re.sub(r'(Current State:\s*\d+):\s+\[', r'\1:[', processed_path)
  
  # Also add a new line after "Goal Reached"
  processed_path = re.sub(r'\b(Reached)\b', r'\1\n', processed_path)
...
def extract_final_answer(search_path):
    # Extracting the target and initial numbers from the first line
    first_line = search_path.strip().split('\n')[0]
    if "<|endoftext|>" in search_path:
      search_path = search_path.replace("<|endoftext|>", "")
    ...
    target_nums_match = re.match(r"Current State: (\d+):\[(.*?)\]", first_line)
    if not target_nums_match:
        return False, "Invalid input: Cannot find the initial state in the first line."
    ...
    goal_lines = list(goal_lines)
    if not goal_lines:
        return False, "No goal reached statement found."
    ...
    operations = re.findall(r"Exploring Operation: (.*?=\d+), Resulting Numbers: \[(.*?)\]",
                            search_path[:goal_line.start()])
    if not operations:
        return False, "No operations found leading to the goal."
    ...
    try:
        ...
        print("couldnt parse last op", final_operation)
        return False, "Couldnt parse last op"
    if predicted_result != target:
        return False, "Invalid path: Final operation does not result in target."
